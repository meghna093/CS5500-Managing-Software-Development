	...
	public boolean hasNextMessage() {
		...
		try {
			...
			while ((start + MIN_MESSAGE_LENGTH) <= charBuffer.limit()) {
				...
				Message newMsg = new Message(sendUser, recGroup, MessageType.getMessageType(handle), message, null);
				logger.info(() -> "Message: " + newMsg.toString());
				if (newMsg.getType() != MessageType.ACKNOWLEDGE || newMsg.getType() != MessageType.NO_ACKNOWLEDGE) {
					messages.add(newMsg);
				}
